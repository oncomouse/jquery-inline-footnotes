(function (factory) {
	...
	$.inlineFootnotes = function( options ) {
		...
		$('sup[id^="' + opts['footNoteRefID'] + '"]').each(function() {
			...
			var footnoteNumber = $footnoteRef.attr('id').replace(opts['footNoteRefID'],"");

			// Whoever thought it was a good idea to put colons in the IDs of autogenerated footnotes is a real meany:
			var $footnote = $('*[id="'+ opts['footNoteID'] + footnoteNumber + '"]');
			var $newFootnote = $('<span class="inline-footnote closed" id="' + opts['inlineFootNoteID'] + footnoteNumber+'">' + $footnote.html() + '</span>');
			...
			$footnoteLink.click(function(e) {
				...
				$('#' + opts['inlineFootNoteID'] + footnoteNumber).toggleClass('closed opened');
				return false;
			});
		});
		...
		$(window).resize(function() {
			...
			if(test) {
				$('sup[id^="' + opts['footNoteRefID'] + '"]').each(function() { 
					var $footnoteRef = $(this);
					var footnoteNumber = $footnoteRef.attr('id').replace(opts['footNoteRefID'],"");
					$.inlineFootnotes.addCSSRule($stylesheet, '#' + opts['inlineFootNoteID'] + footnoteNumber+':before, #' + opts['inlineFootNoteID'] +footnoteNumber+':after', 'left: ' + ($footnoteRef.offset().left - $footnoteRef.parent().offset().left) + 'px !important;', -1);	
				});
			} else {
				var footnotes = $('span[id^="'+opts['inlineFootNoteID']+'"]');
				...
				while(overlaps.length > 0 && count < opts['maxCollisionCycles']) {
					...
					overlaps.each(function(i) {
						...
						var $footnote = $(this);
						var footnoteNumber = $footnote.attr('id').replace(opts['inlineFootNoteID'],"");
						...
						filteredFootnotes.each(function() {
							...
							var testFootnoteNumber = $testFootnote.attr('id').replace(opts['inlineFootNoteID'],"");
	...
	$.inlineFootnotes.defaults = {
		columnWidth: 1155, // minimum width of the column to display footnotes
		mediaQuery: null, // copy & paste the media query from your CSS if you prefer to do that (and have polyfilled window.matchMedia)
		overlapOffset: 5, // padding between overlapping footnotes
		footNoteID: 'fn:', // base of ID for footnotes
		footNoteRefID: 'fnref:', // base of ID for footnote reference links
		inlineFootNoteID: 'fnote_', // base of the IDs of elements we are attaching
		maxCollisionCycles: 8, // Block out too many collision lops
		DEBUG: false // Display debuggin information?
	};
}));